<template>
  <el-tooltip class="item" effect="dark" content="心动模式" placement="top">
    <div
      ref="floatDrag"
  ...
  mounted() {
    this.isCanDraggable &&
      this.$nextTick(() => {
        ...
        // 设置初始位置
        this.left =
          this.clientWidth - this.floatDragDom.width - this.distanceRight - 30;
        this.top =
          this.clientHeight -
          this.floatDragDom.height -
          this.distanceBottom -
          500;
        this.initDraggable();
      });
  ...
  methods: {
    ...
    checkDraggablePosition() {
      if (this.left + this.floatDragDom.width / 2 >= this.clientWidth / 2) {
        // 判断位置是往左往右滑动
        this.left = this.clientWidth - this.floatDragDom.width - 30;
      } else {
        this.left = 0;
      }
      if (this.top < 0) {
